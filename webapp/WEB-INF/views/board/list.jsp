						...
						<tr>
							...
							<td style="text-align: left; padding-left: ${20*vo.depth}px">
								<c:choose>
									<c:when test="${vo.orderNo > 1}">
										...
										<a href="${pageContext.servletContext.contextPath }/board/view?page=${param.page }&no=${vo.no }&groupNo=${vo.groupNo }&orderNo=${vo.orderNo}&depth=${vo.depth}">${vo.title }</a>
									</c:when>
									<c:otherwise>
										<a href="${pageContext.servletContext.contextPath }/board/view?page=${param.page }&no=${vo.no }&groupNo=${vo.groupNo }&orderNo=${vo.orderNo}&depth=${vo.depth}">${vo.title }</a>
									</c:otherwise>
							...
							<c:choose>
								<c:when test="${authUser.no == vo.userNo }">
									<td><a href="${pageContext.servletContext.contextPath }/board/delete?userNo=${authUser.no }&no=${vo.no}&page=${param.page}" class="del">삭제</a></td>
								</c:when>
							</c:choose>
						</tr>
					</c:forEach>
					...
					<ul>
					    ...
					    <c:choose>
					    	<c:when test="${startPage <= param.page && param.page != 1}">
					    		<li><a href="${pageContext.servletContext.contextPath }/board/list?page=${(param.page-1) }">◀  </a></li> 
					    	</c:when>
					    	<c:otherwise>
					    		<li>◀  </li> 
					    	</c:otherwise>
						...
						<c:forEach begin="${startPage }" end="${startPage+4}" step="1" var="i" varStatus="status">
							<c:choose>
								<c:when test="${totalPage >= i }"> 
									<li class="selected"><a href="${pageContext.servletContext.contextPath }/board/list?page=${i }">${i }</a></li>
								</c:when>
								<c:otherwise>
									<li>${i }</li>
								</c:otherwise>
